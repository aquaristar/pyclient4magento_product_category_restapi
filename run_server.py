from abcmagentocli_server import ABCMagentoRESTClient
...
def runCategory():
    with open('catemast.csv', 'r') as f:
        ...
        for row in reader:
            ...
            categoryXML = """<?xml version="1.0"?>
            <magento_api>
            <entity_id>%s</entity_id>
            <name>%s</name>
            <is_active>1</is_active>
            <parent_id>%s</parent_id>
            </magento_api>""" % (categoryId, urllib.quote(categoryName), parentId)
            print categoryXML
...
""" % (product.find('brand').text, product.find('mfn_Num').text, product.find('sku').text, product.find('name').text, product.find('price').text, product.find('saleprice').text, product.find('availability').text)
    ...
    if category != None:
      ...
      for subCategory in product.findall('Sub_category'):
        ...
        arrCategory.append(categoryXML)
    r = abcmagento.bindProduct2Category(pResponse,arrCategory)
    print r


    image_name = product.find('image').text.split('/')[-1]
    image_path = "/var/www/product_images/"
    ...
    r = abcmagento.updateImageInfo(r, imageUpdateXML)
